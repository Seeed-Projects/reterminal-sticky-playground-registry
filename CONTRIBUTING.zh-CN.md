...
贡献者需要在一个 Pull Request 中同时提交：完整源码、可直接烧录的固件包、项目信息和
展示图片。Sticky 私有网站只读取经过审核并锁定版本的公开仓库内容，再生成卡片和烧录页。
...
公开 Registry 保存源码和可安装固件包；Sticky 网站负责页面样式、USB 串口连接、
烧录界面、域名和正式部署。
...
每个项目在 `integrations/` 下建立一个独立目录，并提供以下完整内容：
...
2. 把完整工程放入 `source/`。
3. 加入源码许可证和项目 README。
4. 在 `assets/` 中加入 Logo 和真实设备效果图。
5. 从本次提交的 `source/` 目录完成编译。
6. 把编译结果整理到 `firmware/<version>/`。
7. 运行 Registry 测试和校验。
8. 把这套固件烧录到真实 reTerminal Sticky 上测试。
9. 提交 Pull Request，并写清测试结果。

一句话总结：源码、固件包、页面资料和实机结果齐全后，再进入审核。
...
一句话总结：社区条目只有达到“有源码、有固件、能直刷”才会进入网站。

## 源码要求

`source/` 必须能够仅依靠本次提交的文件完成编译。它应包含工程构建文件、应用代码、
本地组件、依赖清单或锁定文件、默认配置和许可证。
...
PR 需要包含源码、固件包、项目资料、图片和实机测试结果。GitHub Actions 会依次执行：

1. 检查 Registry 结构和本地固件包。
2. 在干净环境中重新编译每个 ESP-IDF 社区项目。
3. 把重新编译的结果与 PR 中的每个 `.bin` 和 SHA-256 逐一比较。
